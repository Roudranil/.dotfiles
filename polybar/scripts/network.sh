...
function rofi_cmd () {
  ...
  echo -e "$1" | uniq -u | rofi -dmenu -i -p "Wi-Fi SSID" -lines "$LINENUM" \
...
function main () {
    ...
    if [[ "$OPS" =~ 'Disable' ]]; then
      ...
      MSSID=$(echo -en "" | rofi -dmenu -i -p "SSID" -mesg "Enter the SSID of the network" \
        -lines 0 -font "$FONT")

      # manual entry of the PASSWORD
      MPASS=$(echo -en "" | rofi -dmenu -i -password -p "PASSWORD" -mesg \
      ...
      if [ "$MPASS" = "" ]; then
        ...
        if [[ "$OPS" =~ "WPA2" ]] || [[ "$OPS" =~ "WEP" ]]; then
          WIFIPASS=$(echo -en "" | rofi -dmenu -i -password -p "PASSWORD" \
